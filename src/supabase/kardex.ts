...
// Tipos crudos que pueden devolver las relaciones de Supabase con !inner
type ProductoRel = { descripcion: string };
type UsuarioRel = { nombres: string };
...
type RawMovement = {
  ...
  detalle?: string | null;
  estado: number;
  // Supabase puede devolver objeto o array según la relación/config
  productos?: ProductoRel | ProductoRel[] | null;
  usuarios?: UsuarioRel | UsuarioRel[] | null;
};

// Obtener todos los movimientos con información extendida usando la función existente
export async function getMovements(idEmpresa: number): Promise<KardexExtendido[]> {
  ...
  try {
    let query = supabase
      ...
      .select(
        ...
        detalle,
        estado,
        productos:productos!inner(descripcion),
        usuarios:usuarios!inner(nombres)
      `
      )
    ...
    // ✅ No compares con '', solo verificá si existe
    if (filters.tipo) {
    ...
    // Tipamos el resultado de la query con joins
    const rows = (data ?? []) as unknown as RawMovement[];

    // Normalizar relaciones que pueden venir como array u objeto
    const getDescripcion = (prod?: ProductoRel | ProductoRel[] | null) =>
      Array.isArray(prod) ? (prod[0]?.descripcion ?? '') : (prod?.descripcion ?? '');

    const getNombres = (usr?: UsuarioRel | UsuarioRel[] | null) =>
      Array.isArray(usr) ? (usr[0]?.nombres ?? '') : (usr?.nombres ?? '');
    ...
    return rows.map((movement) => ({
      ...
      descripcion: getDescripcion(movement.productos),
      nombres: getNombres(movement.usuarios),
...
async function updateProductStock(productId: number, tipo: 'entrada' | 'salida', cantidad: number) {
  try {
...
export async function createMovement(
  ...
  try {
    ...
    // Actualizar stock del producto
    await updateProductStock(movementData.id_producto, movementData.tipo, movementData.cantidad);
...
function groupByDate(movements: Array<{ fecha: string; cantidad: number }>) {
...
export async function getKardexStats(idEmpresa: number, days: number = 30) {
  try {
    ...
    return {
      ...
      entradasPorDia: groupByDate(entradas),
      salidasPorDia: groupByDate(salidas),
    };
  } catch (err) {
...
export async function deleteMovement(id: number) {
  try {
    ...
    // Revertir el stock
    const revertType: 'entrada' | 'salida' = movement.tipo === 'entrada' ? 'salida' : 'entrada';
    await updateProductStock(movement.id_producto, revertType, movement.cantidad);
...
// Para mantener compatibilidad con el código existente, exportamos también un objeto KardexService
